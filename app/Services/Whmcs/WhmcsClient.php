...
class WhmcsClient
{
    ...
    /**
     * Send a POST request to the custom WHMCS DNS proxy endpoint.
     * This calls dns_api.php on the WHMCS server which interfaces
     * with the registrar module's GetDNS/SaveDNS functions.
     *
     * @param string $action  DNS action: GetDNS or SaveDNS
    ...
    public function callDnsProxy(string $action, array $params = []): array
    {
        ...
        try {
            $response = Http::timeout($this->timeout)
                ...
                ->asForm()
                ->post("{$this->baseUrl}/dns_api.php", $payload);
            ...
            if (!$data || !is_array($data)) {
                ...
                throw new WhmcsApiException(
                    'Invalid response from DNS management service',
                    $action,
                    $this->redactParams($params)
                );
            }
            ...
            if (($data['result'] ?? '') === 'error') {
                $rawMsg = $data['message'] ?? 'Unknown DNS error';
                Log::warning("WHMCS DNS Proxy error", [
                    'action'  => $action,
                    'message' => $rawMsg,
                ]);
                throw new WhmcsApiException(
            ...
            Log::error("WHMCS DNS Proxy connection failed", ['action' => $action, 'error' => $e->getMessage()]);
            throw new WhmcsApiException(
                'Unable to connect to DNS management service. Please try again later.',
                $action,
                $this->redactParams($params)
            );
        } catch (\Exception $e) {
            Log::error("WHMCS DNS Proxy unexpected error", ['action' => $action, 'error' => $e->getMessage()]);
            throw new WhmcsApiException(
                'An unexpected error occurred with DNS management. Please try again later.',
                $action,
                $this->redactParams($params)
            );
        }
